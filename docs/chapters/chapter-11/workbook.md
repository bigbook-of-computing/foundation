# **Chapter 11: Parabolic PDEs (Workbook)**

---

> **Summary:** This workbook explores the physics of **Diffusion** and transient heat flow. We transition from static equilibrium to dynamic fields, where the Laplacian ($\nabla^2$) is coupled to a first-order time derivative. You will master the **FTCS** explicit scheme, confront the **CFL stability limit**, and implement the unconditionally stable **Crank-Nicolson** method—the gold standard for diffusion simulations.

---

## **11.1 The Physics of "Spreading Out"** {.heading-with-pill}

> **Difficulty:** ★★★☆☆
> 
> **Concept:** Diffusion and Parabolic PDEs
> 
> **Summary:** We define the Heat/Diffusion Equation ($\partial_t T = D \nabla^2 T$) as the governing law of "spreading" phenomena. This section bridges IVP (time-marching) and BVP (spatial grids) techniques.
...
??? info "See Answer"
        **Correct: B**  
        Parabolic PDEs describe systems that "spread" or diffuse over time.
...
??? info "See Answer"
        **Correct: C**  
        It requires both initial conditions (time) and boundary conditions (space).
...
## **11.2 Discretizing Time and Space** {.heading-with-pill}
...
## **11.3 Method 1: The "Obvious" (and Dangerous) FTCS Method** {.heading-with-pill}
...
??? info "See Answer"
        **Correct: A**  
        Using present values makes the formula explicit and easy to solve directly.
...
??? info "See Answer"
        **Correct: B**  
        This dimensionless "diffusion number" controls the stability of the simulation.
...
**Sample Output:**
```python
WARNING: α = 0.7500 > 0.5 → Instability expected (explosive growth likely).
 Numerical oscillation detected — stopping simulation early.
```
...
??? info "See Answer"
        **Correct: B**  
        Von Neumann analysis proves the solution explodes if this is violated.
...
??? info "See Answer"
        **Correct: B**  
        The $h_x^2$ dependence in the denominator leads to a quadratic increase in computation time.
...
!!! abstract "Interview-Style Question"
    **Question:** A physics student chooses a fixed spatial grid ($h_x$) and a fixed time step ($h_t$) to simulate heat flow, but the solution explodes. They immediately try reducing the time step by $10\%$. Explain why this minor reduction is a flawed strategy and what they should have done instead.
    ...
    ???+ info "Answer Strategy"
        The reduction is flawed because if the solution exploded, the CFL condition ($\alpha \le 0.5$) was violated. The student needs to reduce $h_t$ drastically, not slightly. If the $\alpha$ was, for example, $1.0$, they need to reduce $h_t$ by at least 50% just to reach the *limit* of stability. The correct strategy is to **calculate the maximum stable $h_t$** ($h_{t, \text{max}} = 0.5 \cdot h_x^2 / D$) and choose a time step slightly smaller than that value, ensuring stability.

\<hr\>

## **11.5 Method 2: The "Safe" Implicit Method (BTCS)** {.heading-with-pill}
...
## **11.6 Method 3: The "Gold Standard" Crank-Nicolson** {.heading-with-pill}
...
## **11.7 Core Application: The 1D "Quenched" Rod** {.heading-with-pill}
...
## **11.8 Chapter Summary & Next Steps** {.heading-with-pill}
...
## **11.9 Comprehensive Review** {.heading-with-pill}
...
??? info "See Answer"
        **Correct: B**  
        Diffusion is the defining behavior of Parabolic PDEs.
...
??? info "See Answer"
        **Correct: C**  
        Central difference ensures second-order spatial accuracy.
...
??? info "See Answer"
        **Correct: A**  
        Violating this causes exponential amplification of errors.
...
??? info "See Answer"
        **Correct: C**  
        Implicit schemes "look ahead," coupling future points together.
...
??? info "See Answer"
        **Correct: B**  
        Decoupling stability from step size is the key to efficiency.
...
??? info "See Answer"
        **Correct: B**  
        Averaging the stencils gives second-order accuracy in both time and space.
...
??? info "See Answer"
        **Correct: C**  
        This is the "bad news" of the $h_x^2$ dependence.
...
??? info "See Answer"
        **Correct: C**  
        Each point $i$ is only linked to $i-1$ and $i+1$.
...
??? info "See Answer"
        **Correct: B**  
        Steady-state means temporal change is zero, leaving only the Laplacian.
...
??? info "See Answer"
        **Correct: C**  
        Specialized linear algebra is the engine room of implicit solvers.
...
## **11.10 Hands-On Projects** {.heading-with-pill}
